...
https://web-sprint-challenge-unit4-bpn.herokuapp.com/
...
- The core features of Node.js are as follows
  a. Npm(The Node Package): Node has access to Npm modules, the largest ecosystem of useful libraries which are mostly free.
  b Asynchronous: The Api calls doesn't block other calls or wait for the results.
  c. Single threaded: Node can handle more requests even after running on single thread with event loopings.

The core features of Express are as follows
a. creating own Express Middleware
b. Routing
c. Request / Response feature

1. Understand and explain the use of Middleware.

- Middleware are functions that helps to extend the functionality of the Api. Middleware functions have access to request object, response object and also next function in the application's request - response cycle.

1. The basic principles of the REST architectural style.

- The basic principles of the REST architectural style is as follows
  a. Everything is resource
  b. Each resource is accessible via a unique URL
  c. Resources can have multiple representations
  d. Communication happens over a stateless Protocol(HTTP)
  e. resource management happen via HTTP methods

1. Understand and explain the use of Express Routers.

- Express routers breaks the application to different requst handlers for each Url route and HTTP method combinations.

1. Describe tooling used to manually test the correctness of an API.

- There are several tools like HTTPie, Insomnia, Postman etc which can be used to manually test the correctness of an APT. We can add headers, add JSON data to body, check the responses etc.
